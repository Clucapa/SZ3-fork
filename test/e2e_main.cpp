...
static TestResult run_qoi_test(const QoiDef &qd, uint N, int algo, uint8_t interp_algo,
    ...
    using namespace SZ3;
    TestResult r;
    size_t num = dims[0]; if (N >= 2) num *= dims[1]; if (N >= 3) num *= dims[2];
    ...
    // Overflow check using hardcoded feval
    if (qd.feval) {
        bool overflow = false;
        for (size_t i : {size_t(0), num/4, num/2, 3*num/4, num-1}) {
            if (i >= num) continue;
            if (!std::isfinite(qd.feval(data[i]))) { overflow = true; break; }
        }
        if (qd.feval2) {
            for (size_t i : {size_t(0), num/4, num/2, 3*num/4, num-1}) {
                if (i >= num) continue;
                if (!std::isfinite(qd.feval2(data[i]))) { overflow = true; break; }
            }
        }
    ...
    if (qd.is_regional) {
        // Regional: hardcoded aggregate constraint (mean / mean-of-squares)
        double agg_val = 0;
        if (!check_regional_aggregate(qd, num, data.data(), dec, &agg_val)) {
            r.passed = false; r.fail_reason = "qoi-regional";
        ...
        // Pointwise: hardcoded f(x) per-point check
        size_t fails = 0;
        if (!check_hardcoded_comply(qd.feval, qd.qEB, data.data(), dec, num, &fails)) {
            ...
            r.max_abs = max_abs_err(data.data(), dec, num);
            r.max_qoi = max_hardcoded_qoi(qd.feval, data.data(), dec, num);
        } else {
            r.max_abs = max_abs_err(data.data(), dec, num);
            r.max_qoi = max_hardcoded_qoi(qd.feval, data.data(), dec, num);
        }
        // Second group (MultiQoI)
        if (r.passed && qd.feval2) {
            if (!check_hardcoded_comply(qd.feval2, qd.qEB2, data.data(), dec, num, &fails)) {
                r.passed = false; r.fail_reason = "qoi-violation"; r.qoi_fails = fails;
                r.max_qoi2 = max_hardcoded_qoi(qd.feval2, data.data(), dec, num);
            }
        }
    }
...
static TestResult run_encoder_test(const EncoderTestCase &tc, uint N, int algo,
    ...
    if (!er.ok) { r.passed = false; r.fail_reason = "encoder-error"; r.detail = er.error; return r; }
    ...
    if (algo == TALGO_INTERP || algo == TALGO_INTERP_LORENZO) conf.interpAlgo = interp_algo;
    ...
    if (tc.feval2) {
        if (r.passed && !check_hardcoded_comply(tc.feval2, tc.qEB2, data.data(), dec, num, &fails)) {
            r.passed = false; r.fail_reason = "qoi-violation"; r.qoi_fails = fails;
            r.max_qoi2 = max_hardcoded_qoi(tc.feval2, data.data(), dec, num);
        }
    }
...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --interp-only   Only ALGO_INTERP + ALGO_INTERP_LORENZO\n"
...
int main(int argc, char **argv) {
    ...
    if (!compose_only) {
        ...
        for (int di = 0; di < num_qois(); ++di) {
            ...
            for (uint N = 1; N <= 3; ++N) {
                std::array<size_t, 3> dims;
                dims[0] = dim_size(N, 512); dims[1] = dim_size(N, 32); dims[2] = dim_size(N, 18);
                for (int pi = 0; pi < num_data_patterns(); ++pi) {
                    int pat = ALL_PATTERNS[pi];
                    // Block
                    if (!algo_mask || (algo_mask & (1<<TALGO_BLOCK))) {
                        if (fast && !fast_filter(qd, N, pat)) goto nxt;
                    ...
                    if ((!algo_mask || (algo_mask & (1<<TALGO_INTERP))) && N <= 2) {
                        for (auto ia : interp_algos) {
                            if (fast && ia == SZ3::INTERP_ALGO_LINEAR && !fast_filter_interp_linear(qd,N,pat)) continue;
        ...
        for (int ti = 0; ti < enc_count; ++ti) {
            ...
            for (int pat : enc_patterns) {
                for (int algo : enc_algos) {
                    ...
                    if (fast && algo == TALGO_INTERP) continue;
    ...
    if (verbose && skipped > 0) printf("Skipped: domain mismatch or QOI overflow\n");
    return failed ? 1 : 0;
}
