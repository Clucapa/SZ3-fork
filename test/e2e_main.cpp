...
// --fast: INTERP_LORENZO and LINEAR only for critical QOIs
...
static TestResult run_one(const QoiDef &qd, uint N, int algo, uint8_t interp_algo,
    ...
    // Check QOI constraint: regional → aggregate, pointwise → per-point
    if (qd.is_regional) {
        double agg_val = 0;
...
static void usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --fast          Pruned test suite\n"
        "  --full          Full test suite [default]\n"
        "  --interp-only   Only ALGO_INTERP\n"
...
int main(int argc, char **argv) {
    ...
    // Interpolation algorithms to test: CUBIC (default) and LINEAR
        ...
        for (uint N = 1; N <= 3; ++N) {
            ...
            for (int pi = 0; pi < num_data_patterns(); ++pi) {
                ...
                if (!algo_mask || (algo_mask & (1 << TALGO_BLOCK))) {
                    ...
                    if (r.fail_reason && (!strcmp(r.fail_reason, "skip-domain") || !strcmp(r.fail_reason, "skip-overflow"))) {
                        skipped++;
                    } else if (r.passed) {
                        passed++;
                        if (verbose) printf("PASS [%s] N=%u %s %s: cmp=%zu\n", qd.name, N, pattern_name(pat), "Block", r.cmp_size);
                    } else {
                        fprintf(stderr, "FAIL [%s] N=%u %s %s: %s\n", qd.name, N, pattern_name(pat), "Block", r.fail_reason);
                    }
                }
                skip_block: (void)0;

                // ----- TALGO_INTERP (CUBIC + LINEAR) -----
                if (!algo_mask || (algo_mask & (1 << TALGO_INTERP))) {
                    ...
                    for (auto ia : interp_algos) {
                        ...
                        if (r.fail_reason && (!strcmp(r.fail_reason, "skip-domain") || !strcmp(r.fail_reason, "skip-overflow"))) {
                            ...
                            passed++;
                            if (verbose) printf("PASS [%s] N=%u %s %s: cmp=%zu\n", qd.name, N, pattern_name(pat), algo_name(TALGO_INTERP, ia), r.cmp_size);
                        } else {
                            fprintf(stderr, "FAIL [%s] N=%u %s %s: %s\n", qd.name, N, pattern_name(pat), algo_name(TALGO_INTERP, ia), r.fail_reason);
                        }
                    }
                }
                skip_interp: (void)0;

                // ----- TALGO_INTERP_LORENZO (CUBIC + LINEAR, only 1D/2D) -----
                if (!algo_mask || (algo_mask & (1 << TALGO_INTERP_LORENZO))) {
                    ...
                    for (auto ia : interp_algos) {
                        ...
                        if (r.fail_reason && (!strcmp(r.fail_reason, "skip-domain") || !strcmp(r.fail_reason, "skip-overflow"))) {
                            ...
                            passed++;
                            if (verbose) printf("PASS [%s] N=%u %s %s: cmp=%zu\n", qd.name, N, pattern_name(pat), algo_name(TALGO_INTERP_LORENZO, ia), r.cmp_size);
                        } else {
                            fprintf(stderr, "FAIL [%s] N=%u %s %s: %s\n", qd.name, N, pattern_name(pat), algo_name(TALGO_INTERP_LORENZO, ia), r.fail_reason);
                        }
                    }
                }
                skip_ilo: (void)0;
