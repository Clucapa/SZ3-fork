...
// All QOI verification uses hardcoded f(x) functions.
// Usage: ./e2e [--fast|--compose|--interp-only|--block-only|--verbose] [--encoder-path=PATH]
...
bool call_encoder(const std::string &expr, int &qoi_out,
    ...
    if (out.find("Error:") != std::string::npos || out.find("FX Error:") != std::string::npos) {
        error_out = out; while (!error_out.empty() && (error_out.back()=='\n'||error_out.back()=='\r')) error_out.pop_back();
        return false;
    }
...
static void usage(const char *prog) { fprintf(stderr,
    "Usage: %s [--fast|--compose|--interp-only|--block-only|--verbose] [--encoder-path=PATH]\n", prog); }
static bool is_skip(const TestResult &r) { return r.fail_reason && (!strcmp(r.fail_reason,"skip-domain")||!strcmp(r.fail_reason,"skip-overflow")); }

int main(int argc, char **argv) {
    bool fast=false, verbose=false, compose_only=false; int algo_mask=0;
    ...
    auto run=[&](const char *l,int N,int p,int a,uint8_t i,const TestResult &r){
        ...
        if (!strcmp(r.fail_reason,"qoi-violation")) fprintf(stderr," (%zu pts abs=%.4g qoi=%.4g)",r.qoi_fails,r.max_abs,r.max_qoi);
    ...
    if (compose_only||!fast){
        int n=0; auto ts=all_encoder_tests(n); const int ps[]={D1_RAMP,D3_SINUSOID}; const int as[]={TALGO_BLOCK,TALGO_INTERP};
        std::array<size_t,3> ds={512,32,18};
        for (int ti=0; ti<n; ++ti){ auto &tc=ts[ti];
            for (int p:ps) for (int a:as){
                if (algo_mask&&!(algo_mask&(1<<a))) continue; if (fast&&a==TALGO_INTERP) continue;
                run(tc.label,1,p,a,SZ3::INTERP_ALGO_CUBIC,run_encoder_test(tc,1,a,SZ3::INTERP_ALGO_CUBIC,p,ds));
            }
        }
    }
