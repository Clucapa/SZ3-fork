...
// Maps QOI identifiers to test parameters and builds SZ3 Config objects.
...
// Internal algo codes for the test parameter loop (avoid collision with SZ3::ALGO_*).
constexpr int TALGO_BLOCK          = 0;
constexpr int TALGO_INTERP         = 1;
constexpr int TALGO_INTERP_LORENZO = 2;

struct QoiDef {
    int id;                // conf.qoi  value
    const char *name;      // human-readable label
    QoiDomain domain;      // data domain constraint
    bool is_regional;      // true → aggregate check; false → per-point check
    double qEB;            // QOI tolerance (tau)
    double absErrorBound;  // generous global bound, let QOI's per-point eb do the constraining
};

// Registry of all 19 QOIs: 12 base pointwise, 3 composite, 4 regional.
inline const QoiDef *all_qois() {
    static const QoiDef list[] = {
        ...
        // ---- composite pointwise ----
        {0x12, "SumQoI",    DOM_UNRESTRICTED, false, 1.0,  10.0},   // XCubic + X2
        {0x1F3,"MultiQoI",  DOM_UNRESTRICTED, false, 1.0,  10.0},   // XSqrt AND X2
        {0x14E,"Compose",   DOM_UNRESTRICTED, false, 1.0,  10.0},   // Exp(X2) = e^(x²)
        // ---- regional (aggregate constraints) ----
...
// Dimension sizes used per N (sized to span several blocks/prediction levels).
inline size_t dim_size(uint N, size_t dft1) {
    switch (N) {
        case 1: return dft1;  // 512 for 1D
        case 2: return 32;    // 32×32 = 1024
        case 3: return 18;    // 18³  = 5832
        default: return dft1;
    }
}

// Build a SZ3 Config for one test case.
...
// Quick domain check: skip test if data violates QOI's expected input range.
...
// Verify regional aggregate constraint after decompression.
// For RegMean/RegAvgInt: |mean(orig) - mean(dec)| ≤ qEB.
// For RegMeanSq/RegMeanSqI: |mean(orig²) - mean(dec²)| ≤ qEB.
// Returns true if satisfied; *agg_val receives the computed aggregate error.
inline bool check_regional_aggregate(const QoiDef &qd, size_t n,
    ...
    int rid = ~qd.id;
    if (rid == 0 || rid == 2) {
        // Mean-based: sum(orig - dec) / N
        for (size_t i = 0; i < n; ++i)
            sum_err += orig[i] - dec[i];
        *agg_val = std::fabs(sum_err) / n;
        return *agg_val <= qd.qEB * (1.0 + 1e-8);
    } else {
        // Mean-of-squares based: sum(orig² - dec²) / N
        for (size_t i = 0; i < n; ++i)
